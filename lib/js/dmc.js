...
function setRanAuthBg() {
    ...
    document.getElementById('dmc').style.color = 'rgba('+getTextColor(color)+',0.75';
...
function setRanElemBg(elemId) {
    ...
    document.getElementById(elemId).style.color = getTextColor(color);
}
...
function setElemBg(elemId, color) {
    ...
    document.getElementById(elemId).style.color = getTextColor(dmcColor);
}
...
function setCssColors(bgVar, textVar, sessionColor) {
    let r = document.querySelector(':root');
    let rs = getComputedStyle(r);
    let bgColor;
    if(sessionColor > 0) {
        bgColor = colors[sessionColor];
    } else {
        bgColor = getRandomColor();
    }
    r.style.setProperty(bgVar, bgColor.hex);
    r.style.setProperty(textVar, getTextColor(bgColor));
}