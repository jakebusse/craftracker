...
// Checkboxes
document.addEventListener('DOMContentLoaded', function() {
  const headerCheckbox = document.querySelector('.header .td-check input[type="checkbox"]');
  const checkboxes = document.querySelectorAll('.td-check input[type="checkbox"]');
  const rows = document.querySelectorAll('.inventory-table tr:not(.header)');
...
function openMobileActionsMenu() {
    document.getElementById('inventory-actions').classList.add('mobile-open');
}

function closeMobileActionsMenu() {
    document.getElementById('inventory-actions').classList.remove('mobile-open');
}